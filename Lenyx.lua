...
Linux.Themes = {
    Default = {
        ...
        GradientStart = Color3.fromRGB(0, 120, 255),
        GradientEnd = Color3.fromRGB(25, 25, 112)
    },
    Yellow = {
        Primary = Color3.fromRGB(255, 193, 7),
        Secondary = Color3.fromRGB(255, 152, 0),
        GradientStart = Color3.fromRGB(255, 193, 7),
        GradientEnd = Color3.fromRGB(255, 152, 0)
    },
    Red = {
        Primary = Color3.fromRGB(244, 67, 54),
        Secondary = Color3.fromRGB(183, 28, 28),
        GradientStart = Color3.fromRGB(244, 67, 54),
        GradientEnd = Color3.fromRGB(183, 28, 28)
    },
    Purple = {
        Primary = Color3.fromRGB(156, 39, 176),
        Secondary = Color3.fromRGB(74, 20, 140),
        GradientStart = Color3.fromRGB(156, 39, 176),
        GradientEnd = Color3.fromRGB(74, 20, 140)
    }
}
...
function Linux:GetThemeColor(colorType)
    local theme = self.Themes[self.CurrentTheme] or self.Themes.Default
    return theme[colorType] or theme.Primary
end

function Linux:UpdateThemeElements()
    for _, elementData in pairs(self.ThemeElements) do
        if elementData.Element and elementData.Element.Parent then
            local theme = self.Themes[self.CurrentTheme] or self.Themes.Default
            
            if elementData.Type == "Background" then
                elementData.Element.BackgroundColor3 = theme.Primary
            elseif elementData.Type == "Gradient" then
                elementData.Element.Color = ColorSequence.new{
                    ColorSequenceKeypoint.new(0, theme.GradientStart),
                    ColorSequenceKeypoint.new(1, theme.GradientEnd)
                }
            elseif elementData.Type == "Fill" then
                elementData.Element.BackgroundColor3 = theme.Primary
            elseif elementData.Type == "Progress" then
                elementData.Element.BackgroundColor3 = theme.Primary
            elseif elementData.Type == "Toggle" then
                if elementData.Element:GetAttribute("State") then
                    elementData.Element.BackgroundColor3 = theme.Primary
                end
            end
        end
...
function Linux:SetTheme(themeName)
    if self.Themes[themeName] then
        self.CurrentTheme = themeName
        self:UpdateThemeElements()
    end
end
...
function Linux:Notify(config)
    ...
    local ProgressFill = Linux.Instance("Frame", {
        Parent = ProgressBar,
        BackgroundColor3 = Linux:GetThemeColor("Primary"),
    ...
    table.insert(Linux.ThemeElements, {
    ...
    local success, result = pcall(function()
        if readfile and isfile(Linux.ConfigFolder .. "/" .. name .. ".json") then
            ...
            if configData.Theme then
                Linux:SetTheme(configData.Theme)
            end
...
function Linux.CreateToggleButton()
    local parent = RunService:IsStudio() and LocalPlayer.PlayerGui or game:GetService("CoreGui")
    
    for _, v in pairs(parent:GetChildren()) do
        if v:IsA("ScreenGui") and v.Name == "LinuxToggle" then
            v:Destroy()
        end
    end
    
    local ToggleGui = Linux.Instance("ScreenGui", {
        Name = "LinuxToggle",
        Parent = parent,
        ResetOnSpawn = false,
        Enabled = true,
    ...
    local ToggleButton = Linux.Instance("ImageButton", {
        ...
        ImageSize = UDim2.new(0.6, 0, 0.6, 0),
        ZIndex = 200,
        Active = true,
        Draggable = false
    })
    ...
    local originalSize = UDim2.new(0, 50, 0, 50)
    local pressedSize = UDim2.new(0, 55, 0, 55)
    ...
    ToggleButton.MouseButton1Click:Connect(function()
        if not isPressed then
            ...
            TweenService:Create(ToggleButton, pressInfo, {Size = pressedSize}):Play()
            
            task.wait(0.1)
            
            TweenService:Create(ToggleButton, pressInfo, {Size = originalSize}):Play()
...
function Linux.Create(config)
    ...
    local ToggleButton = Linux.CreateToggleButton()
    local isUIVisible = true
    
    ToggleButton.MouseButton1Click:Connect(function()
        isUIVisible = not isUIVisible
        LinuxUI.Enabled = isUIVisible
    end)
    ...
    if config.Subtitle then
        SubtitleLabel = Linux.Instance("TextLabel", {
            ...
            TextColor3 = Linux:GetThemeColor("Primary"),
            TextSize = 14,
            TextXAlignment = Enum.TextXAlignment.Left,
            AutomaticSize = Enum.AutomaticSize.X,
            ZIndex = 2
        ...
        local SubtitleGradient = Linux.Instance("UIGradient", {
            ...
            Color = ColorSequence.new{
                ColorSequenceKeypoint.new(0, Linux:GetThemeColor("GradientStart")),
                ColorSequenceKeypoint.new(1, Linux:GetThemeColor("GradientEnd"))
            },
            Rotation = 40
        })
        
        table.insert(Linux.ThemeElements, {
            Element = SubtitleGradient,
            Type = "Gradient"
        })
    ...
    CancelButton.MouseButton1Click:Connect(function()
        if not cancelButtonClicked then
            ...
            TweenService:Create(CancelButton, TweenInfo.new(0.2), {BackgroundColor3 = Linux:GetThemeColor("Primary")}):Play()
            task.wait(0.3)
    ...
    ConfirmButton.MouseButton1Click:Connect(function()
        if not confirmButtonClicked then
            ...
            TweenService:Create(ConfirmButton, TweenInfo.new(0.2), {BackgroundColor3 = Linux:GetThemeColor("Primary")}):Play()
            task.wait(0.3)
    ...
    function LinuxLib.Tab(config)
        ...
        local function SelectTab()
            ...
            TabBtn.BackgroundColor3 = Linux:GetThemeColor("Primary")
            ...
            local function UpdateToggle(thisToggle)
                ...
                if thisTrack and thisKnob then
                    ...
                    if currentState then
                        TweenService:Create(thisTrack, tween, {BackgroundColor3 = Linux:GetThemeColor("Primary")}):Play()
    ...
    task.spawn(function()
        ...
        if DefaultTab and Tabs[DefaultTab] then
            ...
            defaultTab.Button.BackgroundColor3 = Linux:GetThemeColor("Primary")
        ...
        elseif tabOrder > 0 then
            ...
            if firstTab then
                ...
                firstTab.Button.BackgroundColor3 = Linux:GetThemeColor("Primary")
    ...
    if config.ConfigSave ~= false then
        ...
        SettingsTab.Section({Name = "Theme Settings"})
        
        local selectedTheme = Linux.CurrentTheme
        
        local themeDropdown = SettingsTab.Dropdown({
            Name = "Theme",
            Description = "Choose your preferred theme",
            Options = {"Default", "Yellow", "Red", "Purple"},
